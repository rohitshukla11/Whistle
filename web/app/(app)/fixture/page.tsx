...
import { CompactPlayerCard, type CardState, type Tint } from "../../../components/PlayerCard";
import { Pitch, type PitchPlayer } from "../../../components/Pitch";
import { QueueOrder } from "../../../components/QueueOrder";
import { SquadTable, type RowAction } from "../../../components/SquadTable";
import { Wallet } from "../../../components/Wallet";
import { FixtureSwitcher } from "../../../components/ui";
import type { Address } from "viem";

import type { ShirtState } from "../../../components/Shirt";
...
const SimPanel = SIM_ON
  ? dynamic(() => import("../../../components/SimPanel").then((m) => m.SimPanel))
  : null;
...
export default function FixturePage() {
  ...
  const { deployment, fixtureId, select, all } = useFixture();
      ...
      {!error && !loading && players.length === 0 && (
        <p className="mb-3 rounded-[12px] border border-line px-3 py-2.5 text-[13px] leading-relaxed text-muted">
          ...
          <span className="mt-1 block text-dim">
            Pick another match above, or run the deploy to mint its squad.
          </span>
        </p>
      )}
